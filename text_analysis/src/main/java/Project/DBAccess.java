	...
	public int[] checkToken(int[] category_counter, String token)
	{
		...
		try {
			while (rs.next())
			{
				...
				if (rs.getString("category_name").contains("Geographie")) category_counter[29]++;
				if (rs.getString("category_name").contains("Luftfahrt")) category_counter[30]++;
				if (rs.getString("category_name").contains("Psychologie")) category_counter[31]++;					
				if (rs.getString("category_name").contains("Terrorismus")) category_counter[32]++;					
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
